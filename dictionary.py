...
def dictionary(emb_file, language=None, use_subword=False, normalize=True):
    try:
        use_subword = FasttextDictionary.Subword.OOV_ONLY if use_subword else FasttextDictionary.Subword.NEVER
...
class FasttextDictionary(MonolingualDictionary):
    ...
    def __init__(self, emb_file, language=None, use_subword=Subword.ALWAYS, normalize=False):
    ...
    def safe_word_vector(self, token):
        oov = token not in self
        if self.use_subword != self.Subword.NEVER or not oov:
            if self.use_subword == self.Subword.ALWAYS or oov:
                v = self.emb.get_word_vector(token)
            else:
                i = self.emb.get_word_id(token)
                v = self.emb.get_input_matrix()[i]
